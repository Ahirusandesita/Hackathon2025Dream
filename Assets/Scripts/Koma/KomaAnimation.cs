...
public class KomaAnimation : MonoBehaviour
{
    ...
    public async UniTask Move(Vector2Int targetPos, PlayerNumber playerNumber)
    {
        // 初期位置
        Vector3 startPos = transform.position;

        // 終了位置
        Vector3 endPos = BanUI.Get().GetWorldPosition(targetPos);

        // 放物線のための経路作成
        // z方向に進む間にy方向で0.5f上がり、また元のy座標に戻る
        Vector3[] waypoints = new Vector3[3];
        waypoints[0] = startPos; // 初期位置
        waypoints[1] = startPos + new Vector3((endPos - startPos).x / 2, _moveAmountY, (endPos - startPos).z / 2); // z方向1.2f進み、y方向に0.5f上がる
